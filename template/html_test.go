...
func Test_Render(t *testing.T) {
	...
	// Single
	buf.Reset()
	engine.Render(&buf, "errors/404", map[string]any{
		"Error": "404 Not Found!",
	})
...
func Test_AddFunc(t *testing.T) {
	...
	// Func is admin
	var buf bytes.Buffer
	engine.Render(&buf, "admin", map[string]any{
		"User": "admin",
	})
...
func Test_Layout_Multi(t *testing.T) {
	...
	for i := 0; i < 2; i++ {
		var buf bytes.Buffer
		err := engine.Render(&buf, "index", map[string]any{
			"Title": "Hello, World!",
		}, "layouts/main")
		expect := `<!DOCTYPE html><html><head><title>Main</title></head><body><h2>Header</h2><h1>Hello, World!</h1><h2>Footer</h2></body></html>`
		result := trim(buf.String())
...
func Test_FileSystem(t *testing.T) {
	engine := NewFileSystem(os.DirFS("./views"), ".html")
